-- ============================================================================
-- Fix `column reference "organization_id" is ambiguous` in
-- upsert_contact_with_org_v2.
-- ============================================================================
-- Background
-- ----------------------------------------------------------------------------
-- The function declares its OUT parameters via RETURNS TABLE (..., organization_id
-- uuid, ...). Inside the function body, an INSERT into organization_domains uses
-- `ON CONFLICT (organization_id, domain) DO NOTHING`. PL/pgSQL refuses to compile
-- that statement because the bare identifier `organization_id` could refer
-- either to the OUT parameter (a PL/pgSQL variable) or to the table column.
--
-- Caught at runtime when an inbound email's domain wasn't in the seed (e.g.
-- fedex.com): the placeholder-org branch was hit, ran the org INSERT, and then
-- the alias INSERT raised the ambiguity error mid-transaction, which the
-- caller (lambda contact_intake.call_upsert_rpc) logged but swallowed —
-- producing emails with NULL contact_id and the structured WARN log line that
-- the CloudWatch alarm triggers on.
--
-- Fix: reference the unique-index target by constraint name instead of column
-- list. The PRIMARY KEY on organization_domains covers exactly
-- (organization_id, domain), so semantics are unchanged.
-- ============================================================================
...
BEGIN
  ...
  --    UNIQUE constraint, so we serialize on a transaction-scoped advisory
  --    lock keyed by the email domain. Two concurrent intake calls for the
  --    same brand-new domain take this lock in turn; the second one
  --    re-resolves through the alias table after the first commits and
  --    finds the org without creating a duplicate row.
  IF v_org_id IS NULL THEN
    ...
    IF v_org_id IS NULL THEN
      ...
      -- Reference the PK by constraint name to avoid ambiguity between the
      -- OUT param `organization_id` (declared in RETURNS TABLE above) and
      ...
      ON CONFLICT ON CONSTRAINT organization_domains_pkey DO NOTHING;
    END IF;
  END IF;
